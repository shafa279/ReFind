...
class _ItemDetailsScreenState extends State<ItemDetailsScreen> {
  ...
  bool _isReporter() {
    final currentUser = AuthService.userId?.trim();
    final reporterId =
        _item?['reporter_id']?.toString().trim();

    debugPrint('CURRENT USER: $currentUser');
    debugPrint('ITEM REPORTER: $reporterId');
  ...
  Future<Map<String, dynamic>?> _getMyPendingClaim() async {
    ...
    for (final claim in claims) {
      ...
      final claimantId =
          claimMap['claimant_id']?.toString().trim();
